...
bool CFTPClient::OpenControlChannel(const tstring& strServerHost, USHORT ushServerPort/*=DEFAULT_FTP_PORT*/)
{
   ...
   try
   {
	  cout << "open " << m_apSckControlConnection.get() << endl;
      ...
      cout << "openB " << m_apSckControlConnection.get() << endl;
   }
...
{
	bool c = SOCKET()!=INVALID_SOCKET;
	bool b = m_apSckControlConnection->operator SOCKET()!=INVALID_SOCKET;
	cout << "connA " << m_apSckControlConnection.get() << " " << b << " " << c << " " << SOCKET() << " " << INVALID_SOCKET << endl;
   return b;
}
...
bool CFTPClient::Login(const CLogonInfo& logonInfo)
{
   ...
   if( !OpenControlChannel(strTemp, ushPort) )
      return false;
cout << "ghi " << m_apSckControlConnection.get() << endl;
   ...
   for ( ; ; )
   {
      ...
      case LO: // LO means we're fully logged on
         if( ChangeWorkingDirectory(mc_strRemoteDirectorySeparator)!=FTP_OK ) {
            ...
            cout << "D " << m_apSckControlConnection.get() << endl;
         return true;
      }
   }
   //return false;
...
{
	cout << "download " << m_apSckControlConnection.get() << endl;
...
/*static*/ bool CFTPClient::TransferFile(const CFTPClient& SourceFtpServer, const tstring& strSourceFile, 
   ...
   if( !TargetFtpServer.SendCommand(CCommand::STOR(), strTargetFile, ReplyTarget) ||
	   ...
	   cout << "reply " << ReplyTarget.Code().Value() << endl;
      return false;}
   cout << "transF" << endl;
   CReply ReplySource;
...
cout << "repTypeA" << endl;
   ...
   for (int i = 0; i < Arguments.size(); i++) {
	   cout << i << ": " << Arguments.at(i) << endl;
   }
cout << CCommand::TYPE().AsString() << endl;
   if( !SendCommand(CCommand::TYPE(), Arguments, Reply) ) {
	   cout << "repTypeC " << Reply.Code().Value() << "bla" << endl;
      return FTP_ERROR;}
   cout << "repTypeD" << endl;
   return SimpleErrorCheck(Reply);
}
